...
type SocialBrandLogoProps = SVGProps<SVGSVGElement> & {
  idPrefix?: string;
  platform: SocialBrand;
};

type SocialBrandLogoTileProps = {
  className?: string;
  idPrefix?: string;
...
export function FacebookBrandLogo({ idPrefix = "facebook-icon", ...props }: SVGProps<SVGSVGElement> & { idPrefix?: string }) {
  const clipPathId = `${idPrefix}-clip`;

  return (
    <svg aria-hidden="true" focusable="false" viewBox="0 0 666.667 666.667" {...props}>
      <defs>
...
export function TikTokBrandLogo(props: SVGProps<SVGSVGElement>) {
  return (
    <svg aria-hidden="true" focusable="false" viewBox="0 0 352.28 398.67" {...props}>
      <g>
...
export function SocialBrandLogo({
  ...
  const sharedProps = {
    "data-rda-social-brand-logo": platform,
    className: cn("rda-social-brand-logo", className),
    ...props,
  };

  if (platform === "facebook") {
    return <FacebookBrandLogo idPrefix={idPrefix} {...sharedProps} />;
  }

  if (platform === "instagram") {
    return <InstagramBrandLogo idPrefix={idPrefix} {...sharedProps} />;
  }

  return <TikTokBrandLogo {...sharedProps} />;
}

export function SocialBrandLogoTile({
  className,
  idPrefix,
  logoClassName,
  platform,
}: SocialBrandLogoTileProps) {
  return (
    <span
      aria-hidden="true"
      className={cn(
        "inline-flex size-6 shrink-0 items-center justify-center rounded-sm border border-border bg-background shadow-sm",
        className,
      )}
    >
      <SocialBrandLogo
