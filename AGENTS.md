...
- Two persistent floating widgets share the viewport corners: the ElevenLabs agent owns bottom-right, and the WhatsApp click-to-chat button (`components/site/whatsapp-fab.tsx`, mounted in `app/layout.tsx`) owns bottom-left at a lower z-index. Keep them in opposite corners and **below** dialogs/mobile menus (`--rda-shell-z-dialog` / `--rda-shell-z-mobile-menu` above `--rda-shell-z-widget` / `--rda-shell-z-fab`). Hide both FABs when `body.rda-mobile-menu-open` or `body.is-mobile-nav-open`. Both are hidden from QA visual/content baselines in `tests/support/qa-helpers.ts`.
