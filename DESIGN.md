...
The WhatsApp floating button is the only persistent corner control (course pages swap it for the course action bar on compact viewports while the bar is visible). Keep it bottom-right on every viewport, inset by the safe area, and hidden while the mobile menu or promo dialog is open. It must not cover the header, footer policy/copyright, or a paid-lander form.
...
- **Course fact strip (`data-rda-fact-strip`):** a bordered description list directly under a course H1 answering price, length, approval/payment plan, and next open date (from `lib/course-schedule.ts`). Labels use the small semibold primary-deep style, values use Playfair. It wraps as flex rows so odd counts never leave empty cells. Facts come from `lib/dental-assisting-program.ts` and `lib/course-facts.ts`; never hand-type dates or invent claims there.
- **Dental Assisting Program page (`data-rda-program-page`):** buyer-first order — hero (eyebrow, H1, lede, fact strip, Request a seat + Call, rating line), schedule + payment plan, timeline, skills, outcomes, graduate stories, instructors, reviews, FAQ, then the pre-selected request form. Alternating bands use `.rda-program-band`.
- **Course action bar (`data-rda-action-bar`):** compact viewports only; primary "Request a seat" (primary token), outline Call, and the WhatsApp square (reserved WhatsApp token). Min 48px tap targets, safe-area padding, hidden under menus/dialogs, and it hides the floating WhatsApp FAB while visible.
